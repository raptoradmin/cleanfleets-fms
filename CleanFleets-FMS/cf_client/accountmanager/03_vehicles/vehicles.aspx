<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/CF_Client.Master" CodeBehind="vehicles.aspx.vb" Inherits="cleanfleets_fms.vehicles" %>
...
<asp:Content runat="server" ContentPlaceHolderID="RightColumnContentPlaceHolder">
	...
	<table cellpadding="0" cellspacing="0" style="width: 100%">
		<tr>
			<td>
				<br />
				<!--09/13/2012 IR: Show the add vehicle button when the current user has read/write permissions on the terminal -->
	...
	<asp:SqlDataSource ID="sds_CFV_Vehicles" runat="server" ConnectionString="<%$ ConnectionStrings:CF_SQL_Connection %>"
		SelectCommand="SELECT [IDVehicles], [IDModifiedUser], [EnterDate], [ModifiedDate], [IDProfileAccount], [AccountName], [IDProfileTerminal], [TerminalName], [IDProfileFleet], [FleetName], [EquipmentType], [EquipmentCategory], [VehicleStatus], [CARBGroup], [LicensePlateNo], [ChassisVIN], [ChassisMake], [ChassisModelYear], [UnitNo], [Description], [AnnualMiles], [AnnualHours], [ActualMiles], [ActualHours], [PlannedComplianceDate], [ActualComplianceDate], [BackupStatusDate], [RetireStatusDate], [Notes], [NotesCF] FROM [CFV_Vehicles] WHERE ([IDProfileFleet] = @IDProfileFleet)">
		<SelectParameters>
