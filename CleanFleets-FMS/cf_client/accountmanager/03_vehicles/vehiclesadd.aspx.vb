...
Public Class vehiclesadd
    ...
    Sub UniqueLicensePlateNoValidator(sender As Object, args As ServerValidateEventArgs)
        ...
        strsqlvalidate = "SELECT COUNT(*) FROM CF_Vehicles WHERE UPPER(RTRIM(LicensePlateNo)) = UPPER(RTRIM(@LicensePlateNo)) AND UPPER(RTRIM(LicensePlateNo)) > '' "
        cmd = New SqlCommand(strsqlvalidate, conn)
    ...
    Protected Sub btnInsert_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnInsert.Click
        ...
        If Page.IsValid() Then
            ...
            strsqlinsert += "IDVehicles, ActualRetirementDate, PlannedRetirementDate, EstReplacementCost, IDModifiedUser, ModifiedDate, IDProfileFleet, IDEquipmentType, IDEquipmentCategory, IDVehicleStatus, IDCARBGroup, LicensePlateNo, ChassisVIN, IDChassisMake, ChassisModel, IDChassisModelYear, UnitNo, Description, AnnualMiles, AnnualHours, ActualMiles, ActualHours, GrossVehicleWeight, PlannedComplianceDate, ActualComplianceDate, BackupStatusDate, RetireStatusDate, Notes, NotesCF, LastOpacityTestDate, LicensePlateState, IDWeightDefinition"
            strsqlinsert += ")"
            strsqlinsert += " values ("
            strsqlinsert += "@IDVehicles, @ActualRetirementDate, @PlannedRetirementDate, @EstReplacementCost, @IDModifiedUser, @ModifiedDate, @IDProfileFleet, @IDEquipmentType, @IDEquipmentCategory, @IDVehicleStatus, @IDCARBGroup, @LicensePlateNo, @ChassisVIN, @IDChassisMake, @ChassisModel, @IDChassisModelYear, @UnitNo, @Description, @AnnualMiles, @AnnualHours, @ActualMiles, @ActualHours, @GrossVehicleWeight, @PlannedComplianceDate, @ActualComplianceDate, @BackupStatusDate, @RetireStatusDate, @Notes, @NotesCF, @LastOpacityTestDate, @LicensePlateState, @IDWeightDefinition"
            strsqlinsert += ")"
