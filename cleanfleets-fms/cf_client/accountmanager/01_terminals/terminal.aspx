<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/CF_Client.Master" CodeBehind="terminal.aspx.vb" Inherits="cleanfleets_fms.terminal" %>
...
<asp:Content runat="server" ContentPlaceHolderID="RightColumnContentPlaceHolder">
	...
	<!--09/13/2012 IR: Edited select statement to only return terminals the user has permissions for-->
	<asp:SqlDataSource ID="sds_Profile_Terminal" runat="server" ConnectionString="<%$ ConnectionStrings:CF_SQL_Connection %>"
		SelectCommand="SELECT CFV_Profile_Terminal.[IDProfileTerminal], [TerminalName], [IDProfileAccount], [AccountName], [City], CAST(Telephone1 AS bigint)as Telephone1, [Ext1] FROM [CFV_Profile_Terminal] INNER JOIN CF_UserTerminals ON CFV_Profile_Terminal.[IDProfileTerminal]=CF_UserTerminals.[IDProfileTerminal] AND CF_UserTerminals.[UserId] = @UserId AND CF_UserTerminals.[PermissionLevel] <> 'G' WHERE ([IDProfileAccount] = @IDProfileAccount)">
		<SelectParameters>
