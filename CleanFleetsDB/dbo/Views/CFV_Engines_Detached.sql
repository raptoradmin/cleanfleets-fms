CREATE VIEW [dbo].[CFV_Engines_Detached]
AS
SELECT        IDEngines, IDModifiedUser, EnterDate, ModifiedDate, IDProfileAccount,
                             (SELECT        AccountName
                               FROM            dbo.CF_Profile_Account
                               WHERE        (dbo.CF_Engines.IDProfileAccount = IDProfileAccount)) AS AccountName, IDVehicles, IDEngineManufacturer,
                             (SELECT        DisplayValue
